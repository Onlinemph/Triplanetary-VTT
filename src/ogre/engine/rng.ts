...
/** Roll one six-sided die. Ogre uses nothing else. */
 ...
 *
 * The Size Table (6.05) is the main customer: an Ogre ramming another Ogre
 * rolls one to six dice and the *total* is the tread damage done.
 */
...
/** An integer in [0, n). */
export const nextInt = (state: RngState, n: number): { state: RngState; value: number } => {
  const { state: next, value } = nextFloat(state);
  return { state: next, value: Math.floor(value * n) };
};

/**
 * Deterministic shuffle.
 *
 * Used where the rules say "roll randomly to see which squad is lost" (3.02)
 * and for procedural map generation.
 */

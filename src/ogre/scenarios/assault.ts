 ...
 *
 * Two ScenarioDefs share this build — `assault` on the cratered map and
 * `assault-green` on the settled one — because a `ScenarioDef` names one map
 * and a living world and a dead one are not the same battlefield. The order's
 * documented terms:
 *
 *  - `profile`:  'dead' | 'living' | 'volcanic' | 'asteroid' — the overlay
 *    table of Orbital Drop §5. 'living' belongs to `assault-green`; the rest
 *    to `assault`.
 ...
 *
 * The base is the defender's command post: captured intact when the attacker
 * wins with the CP still standing — which is how the campaign side reads
 * "captured intact" off the result's survivor list.
 */
...
import { createGame, log, makeOgre, makePlayer, makeUnit, withUnit } from '../engine/state.js';
...
/**
 * Roll the world's overlays onto the printed map, as terrain overrides. The
 * doc hands the overlay placement to a player; here the same seed that builds
 * the battle places them, so the battlefield is generated when the invasion
 * lands and identical in every replay.
 */
const rollOverlays = (
  rng: RngState,
  map: GameMap,
  ...
  entryEdge: Edge,
): { rng: RngState; overrides: Record<string, Terrain> } => {
  ...
  if (profile === 'living') {
    ...
    // Dead worlds: craters, placed by the attacker. Io adds lava, which the
    // engine's water already is: impassable to everything, fire crosses it.
    // Asteroid bases stand in with a denser crater field for their ridges.
    const craters = profile === 'asteroid' ? roll(6) + roll(6) : roll(6);
...
const build = (map: GameMap, opts: ScenarioBuildOptions, scenarioId: string): GameState => {
  ...
  const base = createGame({
    ...
    options: {
      ...
      // rules — "no stacking, ramming rules recommended for speed" (§5).
  ...
  const d: Deployer = { state: { ...base, terrainOverrides: rolled.overrides }, serial: 1 };
  ...
  const screen = deployScreened(d, defender.player, defender.forces, central.items, rear.items);
  void screen;
 ...
 * invader's assembly delay — and everything else places like The Landing.
 */
const deployForces = (
  d: Deployer,
  owner: string,
  ...
  hexes: Hex[],
  opts: { ogreActivatesOn?: (type: OgreTypeId) => number } = {},
): void => {
...
const checkVictory = (state: GameState): VictoryState | null => {
  ...
  const cp = units.find((u) => u.kind === 'unit' && u.owner === defender && u.classId === 'CP');
  const baseStands = !!cp && surviving(cp);
...
// ---------------------------------------------------------------------------
// The printed default, and the two scenario defs
// ---------------------------------------------------------------------------
...
const BRIEFING =
  ...
  'The garrison and the base’s militia set up around the command post, no more ' +
  ...
  'defending Ogre starts assembled. Each warship overhead owes the attacker one ' +
  'orbital strike: its combat strength, any target, any range, once per battle.';

const VICTORY_CONDITIONS: readonly string[] = [
  'Attacker destroys the command post: the invasion succeeds, but takes a ruin.',
...
const makeDef = (id: string, name: string, map: GameMap, blurb: string): ScenarioDef => ({
  id,
  name,
  mapId: map.id,
  players: 2,
  ...
  build: (opts) => build(map, opts, id),
  checkVictory,
});

/** Dead, volcanic and asteroid worlds: the cratered map. */
